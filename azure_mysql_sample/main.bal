...
function insertUser(jdbc:Client jdbcClient, string fname, string lname, string country) returns sql:ExecutionResult{
    sql:ParameterizedQuery insertQuery = `INSERT INTO testbal.user(FirstName, LastName, Country) 
...
function getAllUser(jdbc:Client jdbcClient){
    stream<record{}, error> resultStream = jdbcClient->query("Select * from testbal.user");
